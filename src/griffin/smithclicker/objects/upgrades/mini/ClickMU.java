...
public class ClickMU extends Miniupgrade{
	
	private IncreaseType type;
	private BigInteger increase;
	
	public ClickMU(String name, String desc, BigInteger cost, Image img, List<Requirement> requires, IncreaseType type, BigInteger increase, Color color) {
	...
	public void onIncreasedSmiths(IncreaseSmithEvent event) {
		if(event.getType() == FromType.click) {
			switch(type) {
			case base_add:
